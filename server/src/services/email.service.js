...
class EmailService {
  ...
  async sendVerificationEmail(to, token, name = 'there') {
    ...
    const mailOptions = {
      ...
      to,
      subject: 'Verify Your Email - Suicide Note',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    try {
      ...
      winston.info(`Verification email sent to: ${to}`);
      return true;
    } catch (error) {
  ...
  async sendWelcomeEmail(to, name = 'there') {
    const mailOptions = {
      from: process.env.EMAIL_FROM,
      to,
      subject: 'Welcome to Suicide Note!',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    try {
      await this.transporter.sendMail(mailOptions);
      winston.info(`Welcome email sent to: ${to}`);
      return true;
    } catch (error) {
  ...
  async sendAccessCodeEmail(to, name, accessCode, ebook) {
    ...
    const mailOptions = {
      from: process.env.EMAIL_FROM,
      to,
      subject: `Your Access Code for ${ebook.title} - Suicide Note`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
  ...
  async sendPurchaseConfirmationEmail(to, name, purchase, accessCode) {
    ...
    const mailOptions = {
      from: process.env.EMAIL_FROM,
      to,
      subject: 'Purchase Confirmation - Suicide Note',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    try {
      await this.transporter.sendMail(mailOptions);
      winston.info(`Purchase confirmation email sent to: ${to}`);
      return true;
    } catch (error) {
      winston.error('Error sending purchase confirmation email:', error);
      return false;
    }
  }
  ...
  async sendAffiliateWelcomeEmail(to, name, affiliate) {
    const mailOptions = {
      from: process.env.EMAIL_FROM,
      to,
      subject: 'Welcome to the Suicide Note Affiliate Program!',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    try {
      await this.transporter.sendMail(mailOptions);
      winston.info(`Affiliate welcome email sent to: ${to}`);
      return true;
    } catch (error) {
      winston.error('Error sending affiliate welcome email:', error);
      return false;
    }
  }
  ...
  async sendAffiliateCommissionEmail(to, name, purchase, commissionAmount) {
    ...
    try {
      await this.transporter.sendMail(mailOptions);
      winston.info(`Affiliate commission email sent to: ${to}`);
      return true;
    } catch (error) {
      winston.error('Error sending affiliate commission email:', error);
      return false;
    }
  }
  ...
  async sendPasswordResetEmail(to, token) {
    ...
    const mailOptions = {
      from: process.env.EMAIL_FROM,
      to,
      subject: 'Reset Your Password - Suicide Note',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    try {
      await this.transporter.sendMail(mailOptions);
      winston.info(`Password reset email sent to: ${to}`);
      return true;
    } catch (error) {
  ...
  async sendPasswordChangedEmail(to) {
    const mailOptions = {
      from: process.env.EMAIL_FROM,
      to,
      subject: 'Password Changed Successfully - Suicide Note',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    try {
      await this.transporter.sendMail(mailOptions);
      winston.info(`Password changed email sent to: ${to}`);
      return true;
    } catch (error) {
      winston.error('Error sending password changed email:', error);
      return false;
    }
  }
  ...
  async sendRefundConfirmationEmail(to, name, purchase, reason) {
    ...
    const mailOptions = {
      from: process.env.EMAIL_FROM,
      to,
      subject: 'Refund Request Received - Suicide Note',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    try {
      await this.transporter.sendMail(mailOptions);
      winston.info(`Refund confirmation email sent to: ${to}`);
      return true;
    } catch (error) {
      winston.error('Error sending refund confirmation email:', error);
      return false;
    }
  }
  ...
  async sendPayoutConfirmationEmail(to, name, amount, reference) {
    ...
    try {
      await this.transporter.sendMail(mailOptions);
      winston.info(`Payout confirmation email sent to: ${to}`);
      return true;
    } catch (error) {
      winston.error('Error sending payout confirmation email:', error);
      return false;
    }
  }
}
