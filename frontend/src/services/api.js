...
// ─── PAYMENT API (UPDATED FOR SELAR) ───
export const paymentAPI = {
  // SELAR: Get checkout URL
  getSelarCheckout: (params) => api.get('/payments/selar/checkout', { params }),
  
  // SELAR: Verify payment
  verifySelarPayment: (data) => api.post('/payments/selar/verify', data),
  
  // SELAR: Handle redirect (alternative)
  handleSelarRedirect: (params) => api.get('/payments/selar/redirect', { params }),
  
  // Keep for backward compatibility but redirect to Selar
  initialize: (data) => {
    console.warn('⚠️ Payment initialization via Paystack is deprecated. Use getSelarCheckout instead.');
    return api.get('/payments/selar/checkout', { 
      params: {
        email: data.email,
        name: data.name,
  ...
  // Keep verify for compatibility but use Selar verification
  verify: (data) => {
    console.warn('⚠️ Payment verification via Paystack is deprecated. Use verifySelarPayment instead.');
    return api.post('/payments/selar/verify', data);
  },
  
  // Access code validation (still works)
  validateAccessCode: (data) => api.post('/payments/validate-access-code', data),
